...
fn main() -> ! {
    ...
    let mut last_tick = hog_2.get_ticks();

    for _ in 0..20 {
        arb_port.enqueue(&[69; 8]).unwrap();
    }

    loop {
        arb_port.poll().unwrap();

        if hog_2.micros_since(last_tick) > 500 {
            arb_port.query_component().ok();
        }

        if let Some(msg) = arb_port.dequeue() {
            assert_eq!(&msg[..], &[42; 32]);
            msg.release();
            ct += 1;
